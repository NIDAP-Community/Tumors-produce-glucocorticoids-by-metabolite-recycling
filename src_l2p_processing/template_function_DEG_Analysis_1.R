...
DEG_Analysis_1 <- function(Filtered_Counts_1, ccbr1224_rev_response_gsea_meta) {
    ...
    #Basic Parameters:
    counts_matrix <- Filtered_Counts_1 
...
print("template_function_DEG_Analysis_1.R #########################################################################")
library(plotly);library(ggplot2);library(jsonlite);
currentdir <- getwd()
rds_output <- paste0(currentdir,'/rds_output')
var_Filtered_Counts_1<-readRDS(paste0(rds_output,"/var_Filtered_Counts_1.rds"))
Input_is_Seurat_count <- 0
for(item in var_Filtered_Counts_1){ if (class(item)=="Seurat"){Input_is_Seurat_count = Input_is_Seurat_count + 1}}
if(Input_is_Seurat_count == 0 ){
var_Filtered_Counts_1<-as.data.frame(var_Filtered_Counts_1)}else{var_Filtered_Counts_1 <- var_Filtered_Counts_1}
currentdir <- getwd()
rds_output <- paste0(currentdir,'/rds_output')
...
var_DEG_Analysis_1<-DEG_Analysis_1(var_Filtered_Counts_1,var_ccbr1224_rev_response_gsea_meta)
invisible(graphics.off())
saveRDS(var_DEG_Analysis_1, paste0(rds_output,"/var_DEG_Analysis_1.rds"))
